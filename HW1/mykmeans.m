...
%
    ...
    while (iter < 80)
        ...
        for i=1:K    %loop for centroids
            ...
            centroid(i,:) = mean(clu,1); %Take means for each column, which represents the mean value of color channels respectively.
            
            totalCost = totalCost + sum( sqrt(sum( (repmat(clu(j,:),numMember,1)-clu).^2, 2  ))); % Euclidean distance
            % totalCost = totalCost + sum(sum( abs( repmat(centroid(i,:),numMember,1)-clu ) )); % Manhattan distance
